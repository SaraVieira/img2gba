...
class ButanoImgApp(App):
    ...
    def __init__(self):
        ...
        self.num_colors = 256
        self.sprite_height: int | None = None
        ...
        with Container(id="main-container"):
            ...
            with Vertical(id="options-panel"):
                ...
                with Vertical(id="options-container"):
                    ...
                    with Vertical(classes="option-group"):
                        yield Label("Sprite Height (for sprite sheets):", classes="option-label")
                        yield Input(placeholder="Leave empty for single sprite", id="height-input")
                        yield Static("e.g., 32 for 32px tall frames", classes="input-hint")

                    with Vertical(classes="option-group"):
                        yield Label("Compression:", classes="option-label")
                        with RadioSet(id="compression-radio"):
                            yield RadioButton("None", value=True, id="radio-compression-none")
                            yield RadioButton("LZ77", id="radio-compression-lz77")
                            yield RadioButton("Run Length", id="radio-compression-run_length")
                            yield RadioButton("Auto", id="radio-compression-auto")
    ...
    @on(Input.Changed, "#height-input")
    def on_height_changed(self, event: Input.Changed) -> None:
        """Handle sprite height input change."""
        value = event.value.strip()
        if value == "":
            self.sprite_height = None
        else:
            try:
                self.sprite_height = int(value)
                self.log_message(f"Sprite height: [cyan]{self.sprite_height}px[/cyan]")
            except ValueError:
                self.sprite_height = None

    @on(RadioSet.Changed, "#compression-radio")
    def on_compression_changed(self, event: RadioSet.Changed) -> None:
        """Handle compression radio selection."""
        radio_id = str(event.pressed.id)
        if radio_id == "radio-compression-none":
            self.compression = None
        elif radio_id == "radio-compression-lz77":
            self.compression = "lz77"
        elif radio_id == "radio-compression-run_length":
            self.compression = "run_length"
        elif radio_id == "radio-compression-auto":
            self.compression = "auto"

        if self.compression:
            self.log_message(f"Compression: [cyan]{self.compression}[/cyan]")
        else:
            self.log_message("Compression: [cyan]none[/cyan]")
        ...
        try:
            ...
            if result.success:
                ...
                if self.sprite_height:
                    self.log_message(f"  Sprite height: {self.sprite_height}px (sprite sheet)")

                if self.compression:
                    self.log_message(f"  Compression: {self.compression}")
